...
/**
 * Exposes an {@link IEnergyBudget} to the energy systems of other mods and pushes the budget out to neighbouring blocks once per tick.
 */
public final class EnergyBridge {
	/**
	 * Mekanism converts Forge Energy at 2.5 J per FE by default, and the Power Box has always run at 50 FE per kW, so one kW is worth 125 J.
	 */
	...
	/** Builds the Mekanism-side handler for a budget, or an empty optional when Mekanism is not installed. */
	...
	/** A Forge Energy view of a budget, for the mods that do not speak Joules. */
	public static IEnergyStorage forgeEnergyView(IEnergyBudget budget) {
		return new IEnergyStorage() {
			...
			public int extractEnergy(int maxExtract, boolean simulate) {
				// claim exactly the Joules matching the whole FE handed over
	...
	/**
	 * Pushes this tick's budget to the acceptors adjacent to {@code source}, the way a Mekanism generator emits from its energy sides every tick rather than waiting to be pulled from.
	 */
	...
	/**
	 * Mekanism's own handler is preferred over Forge Energy, matching the priority order in Mekanism's EnergyCompatUtils.
	 */
	@Nullable
